...
The current signed candidate is `v2.3.0-rc.3` from revision `7f6c740`.
Its tag workflow
...
corrected the ASP.NET Core named-scheme audience binding before this candidate;
the post-merge workflow
[#145](https://github.com/tov-vl/gold-src-ops/actions/runs/33403763413) and
both candidate image jobs passed.
...
healthy, anonymous protected requests are rejected, and the dedicated Operator
identity reaches the Reader and Operator policy surfaces. A guarded daily backup
schedule, scoped retention policy, and freshness probe are implemented and
container-tested, but live timer activation remains pending. The narrower
Reader-only and signed negative-token matrix, game-server deployment,
observability stack, recovery exercise, and soak evidence also remain pending.
...
2. Completed: publish immutable release-candidate images to GHCR without
   treating them as stable releases. The current signed `v2.3.0-rc.3` tag
...
3. Completed: create a private Backblaze B2 S3-compatible backup repository in
   ...
   encryption is enabled, and Object Lock is deferred until the retention
   policy is defined. On 2026-08-30 the repository was initialized and checked
   with `-ReadDataSubset 100%` using the pinned restic image digest. This proves
   the remote repository bootstrap only; the first PostgreSQL backup and restore
   rehearsal remain step 6.
...
6. In progress: the current candidate is published and digest-pinned;
   ...
   mandatory retention preview and freshness marker; activate it on the target
   and capture the first successful cycle. Complete the dedicated Reader-only,
   missing-role, expired-token, wrong-issuer, and wrong-audience checks before
   closing this step.
...
[#123](https://github.com/tov-vl/gold-src-ops/actions/runs/33305723331)
verified its immutable digest. The current `v2.3.0-rc.3` candidate and workflow
[#146](https://github.com/tov-vl/gold-src-ops/actions/runs/33404280101)
...
`v2.3.0-rc.3` delta contains only deployment configuration, preflight, and
documentation changes, so application and migration source are unchanged.
...
anonymous dashboard and metrics requests returned `401`; an Operator token with
the expected issuer, audience, namespaced role, and subject returned `200` from
the dashboard and metrics surfaces. A missing-server Operator command probe
returned the expected `404` without creating durable state. A dedicated
Reader-only token plus signed missing-role, expired-token, wrong-issuer, and
wrong-audience cases remain pending.
...
- External OAuth 2.0 or OpenID Connect integration for Reader and Operator
  policies. The reference provider and token contract are configured; public
  route verification remains pending.
