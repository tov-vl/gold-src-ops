...
External identity, database recovery gates, and the remaining live evidence are
pending.
...
5. In progress: DNS for `api.goldsrcops.com` resolves to the control-plane host,
   and public certificate issuance has passed with Caddy `2.11.4` pinned as
   `sha256:5f5c8640aae01df9654968d946d8f1a56c497f1dd5c5cda4cf95ab7c14d58648`.
   The API and production Compose contract now support an explicit,
   provider-independent role claim type. Select and configure the external
   identity provider, then verify the public Reader and Operator boundary
   through the tracked Caddy configuration.
