...
the temporary listener was removed and the production runtime remains disabled.
The external Auth0 provider and token contract are configured. Database recovery
gates, public API authorization, and the remaining live evidence are pending.
...
2. Completed: publish immutable release-candidate images to GHCR without
   treating them as stable releases. Signed tag `v2.3.0-rc.1` produced and
   verified digest
   `sha256:d21a5cb10bb8179310e660d50cb301fed277a5dc3fcbd900e77065fdcc9df458`.
...
6. Publish a candidate image from the current `main`, start PostgreSQL, create
   and restore the first off-host backup, apply the image-contained migration
   bundle, and start the API and Caddy only after all gates pass. Verify the
   public Reader, Operator, and rejection boundary through that runtime.
...
[#123](https://github.com/tov-vl/gold-src-ops/actions/runs/33305723331)
verified its immutable digest. Stable promotion remains intentionally unexecuted
until a matching stable release is approved.
...
No production PostgreSQL snapshot exists yet, so this evidence does not satisfy
the backup or restore gates below.
...
target evidence is retained outside Git. This proves DNS and certificate
issuance only; it does not satisfy the forwarded-header, AllowedHosts, readiness,
or authorization checks that require the production API runtime.
...
and token-issuance part of the identity deliverable. Public Reader, Operator,
missing-role, expired-token, wrong-issuer, and wrong-audience behavior remains a
separate runtime verification after the recovery and migration gates pass.
