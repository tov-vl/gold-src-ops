...
and local target-runtime evidence. Slice 3 has started with a provider-independent
Compose and preflight contract under `ops/production`: Caddy is the only public
service, the API trusts one configured proxy address, and PostgreSQL is isolated
behind a shared Unix-domain socket. Infrastructure purchase, pre-purchase
verification, migration and backup actions, provisioning, and all
...
The first implementation sub-slice is tracked in `ops/production`. It requires
digest-pinned API, PostgreSQL, and Caddy images; keeps API and database ports off
the host; injects database and RCON values from external files; and validates the
contract without printing secrets. API and Caddy are intentionally behind the
explicit `runtime` Compose profile. That profile must not be enabled before the
serialized migration action is implemented and successfully applied.
