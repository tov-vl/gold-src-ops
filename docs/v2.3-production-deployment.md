...
Deterministic smoke coverage cannot count as target evidence. Infrastructure
purchase, provisioning, and all live target-environment evidence remain
pending.
...
3. Completed: create a private Backblaze B2 S3-compatible backup repository in
   EU Central and keep the restic key outside the future VPS failure domain.
...
4. In progress: the reviewed two-phase hardening procedure and its audit are
   implemented. Provision the control-plane VPS, apply both phases through
   verified SSH sessions, and collect live host-preflight evidence.
...
recovery escrow outside the future VPS, while backend credentials and sanitized
bootstrap evidence remain outside Git. Repository initialization and an
integrity check configured with a `100%` data subset passed on 2026-08-30. No
production PostgreSQL snapshot exists yet, so this evidence does not satisfy the
backup or restore gates below.
