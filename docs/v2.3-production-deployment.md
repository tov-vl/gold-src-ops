...
120 seconds passed with zero failures and zero bots. The guarded write command,
restart/restore, no-bot observation, observability stack, recovery exercise,
...
7. In progress: the controlled MyArena game VDS, provider-independent host
   ...
   passed with zero failures and zero bots. Trial-period address stability, one
   guarded `say`, ReHLDS restart/restore, and the 24-hour no-bot observation
   remain pending.
