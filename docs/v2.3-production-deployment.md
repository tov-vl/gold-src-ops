...
Architecture and delivery plan accepted on 2026-08-28. Slice 1 implements the
GHCR release-publication and digest-smoke contract. Its first published registry
digest remains future release evidence; infrastructure purchase,
target-provider selection, and target-environment evidence have not started.
...
Status: implemented in the repository. Publication evidence is pending the next
signed release tag.
...
- External A2S_INFO evidence from the future control-plane network.
...
- GoldSrcOps polls the external endpoint repeatedly without relying on loopback.
