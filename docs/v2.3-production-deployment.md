...
gate is intentionally omitted; trial-period address stability, the observability
stack, recovery exercise, and 72-hour soak evidence remain pending.
...
configuration, direct `/metrics` compatibility, and unavailable-endpoint
readiness coverage are implemented. The private Collector, Prometheus, Grafana,
dashboard, and container-level path verification remain pending.
