...
the verification message. Restart/restore, no-bot observation, observability
stack, recovery exercise, and soak evidence remain pending.
...
7. In progress: the controlled MyArena game VDS, provider-independent host
   ...
   passed with its Operator audit identity and terminal state persisted.
   Trial-period address stability, ReHLDS restart/restore, and the 24-hour
   no-bot observation remain pending.
...
seconds with zero failures. Host-specific evidence remains outside Git.
