...
7. In progress: the controlled MyArena game VDS, provider-independent host
   ...
   and a seven-query stability gate have passed. Trial-period address stability,
   endpoint registration and scheduled polling, one guarded `say`, ReHLDS
...
the plugin-free baseline, active firewall, and zero failed units. The game unit
remains disabled and inactive, with no activation marker, runtime configuration,
secret, process, or UDP listener. Host-specific evidence remains outside Git.
