...
Trial-period address stability and 72-hour soak evidence remain pending.
...
7. In progress: the controlled MyArena game VDS, provider-independent host
   ...
   remains part of the Slice 6 72-hour soak evidence.
...
9. In progress: the controlled recovery exercise has passed with sanitized
   root-only evidence. Retain at least 72 hours of soak evidence after the final
   deployment change.
...
- Guarded RCON preflight and one fixed `say` through the authenticated production
  ...
  safety constraints.
- YaPB kept absent through the 72-hour deployment soak and introduced only in a
