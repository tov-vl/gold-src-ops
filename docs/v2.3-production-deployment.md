...
The final soaked candidate is `v2.3.0-rc.5` from revision `58a74da`.
Its tag workflow
...
Trial-period address stability passed. The accepted 24-hour release soak
completed after 24.095 hours with `Passed` target evidence from both hosts.
Final evidence pull request
[#68](https://github.com/tov-vl/gold-src-ops/pull/68), publication hardening pull
request [#69](https://github.com/tov-vl/gold-src-ops/pull/69), and their
post-merge workflows passed. Signed tag `v2.3.0` promotes the verified
`v2.3.0-rc.5` digest, final workflow
[#33788658773](https://github.com/tov-vl/gold-src-ops/actions/runs/33788658773)
passed publication and published-image verification, and the stable GitHub
...
2. Completed: publish immutable release-candidate images to GHCR without
   treating them as stable releases. The final soaked `v2.3.0-rc.5` tag
...
7. Completed: the controlled MyArena game VDS, provider-independent host
   ...
   Trial-period address stability and the integrated 24-hour no-bot observation
   passed. The previously planned standalone 24-hour no-bot observation is
   intentionally omitted; bot count remains part of the integrated Slice 6
   soak evidence.
...
9. Completed: the controlled recovery exercise passed with sanitized root-only
   evidence. The accepted 24-hour soak subsequently completed after 24.095
   hours with all 18 control-plane and eight game-host checks passing.
10. Completed: readiness evidence, terminal SLI values, inactive SLO proposals,
    and rollback references are recorded. Final repository checks passed before
    signed stable tag `v2.3.0` promoted and verified the candidate digest and
...
[#123](https://github.com/tov-vl/gold-src-ops/actions/runs/33305723331)
verified its immutable digest. The final `v2.3.0-rc.5` candidate and workflow
[#204](https://github.com/tov-vl/gold-src-ops/actions/runs/33631016313)
also passed publication and published-image verification. Signed stable tag
`v2.3.0` later promoted that exact digest, and final workflow
[#33788658773](https://github.com/tov-vl/gold-src-ops/actions/runs/33788658773)
verified it before publishing the GitHub Release.
...
Status: complete under the operational contract in
...
bots; the service invocation and process remained unchanged. The later bounded
address comparison and integrated 24-hour soak also passed with zero bots and
unchanged game-host continuity. Host-specific evidence remains outside Git.
...
Status: complete; stable release published. Matching owner-only baselines and
the accepted 24-hour interval produced `Passed` target evidence on both hosts
after 24.095 hours. The control-plane evaluator passed 18/18 checks, the
game-host evaluator passed 8/8 checks and every continuity flag, and
trial-period address stability passed. Sanitized terminal SLI values and
reviewed inactive SLO proposals are recorded. Final repository checks and
published-image verification passed before the stable release was published.
...
achieved SLO.
Deployments, migrations, configuration changes, or process restarts after the
terminal observation fall outside the completed evidence and require a new
baseline before any subsequent soak claim.
...
The v2.3 candidate was released only after:
