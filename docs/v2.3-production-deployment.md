...
identity reaches the Reader and Operator policy surfaces. The narrower
Reader-only and signed negative-token matrix, backup schedule, game-server
deployment, observability stack, recovery exercise, and soak evidence remain
pending.
...
6. In progress: the current candidate is published and digest-pinned;
   ...
   policies are verified. Complete the target backup schedule and the dedicated
   Reader-only, missing-role, expired-token, wrong-issuer, and wrong-audience
   checks before closing this step.
