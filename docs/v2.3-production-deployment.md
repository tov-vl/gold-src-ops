...
7. In progress: the controlled MyArena game VDS, provider-independent host
   ...
   service remains disabled and inactive. A plan-first, stdin-only, rollback-safe
   activation workflow is implemented. Apply it from the reviewed control-plane
   source, then verify external A2S, guarded RCON, address stability,
   source-address preservation, and ReHLDS recovery.
