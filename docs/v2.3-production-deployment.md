...
active on the target; the required preview and first completed cycle passed. The
narrower Reader-only and signed negative-token matrix, game-server deployment,
...
6. In progress: the current candidate is published and digest-pinned;
   ...
   activation, and first completed target cycle have passed. Complete the
   dedicated Reader-only, missing-role, expired-token, wrong-issuer, and
   wrong-audience checks before closing this step.
