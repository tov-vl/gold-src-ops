...
has proved normalization and independent Backblaze B2 archival; alert routing
and a fresh revised-policy shadow remain before `API-01` can advance. The
complete comparison, cost boundary, credential model, and exit conditions are
in the
...
3. **Target setup (in progress)**: the primary and diagnostic shadow checks were
   configured on 2026-09-04. The normalized exporter, create-only local JSONL,
   deterministic evaluator, and contract fixtures are implemented. A bounded
   least-privilege live export has passed. A content-addressed B2 upload,
   ...
   access has verified the expected labels and log format. Alert routing remains
   unproved. The sanitized effective configuration and exporter workflow are in
   `docs/v2.4-synthetic-monitoring-rollout.md` and
   `docs/v2.4-availability-evidence-exporter.md`.
4. **Shadow validation**: after alert proof, collect 24 hours, verify 1,440
   mature evaluated slots and target attainment with missing retained as bad,
