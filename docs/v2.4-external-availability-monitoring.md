...
Decision date: 2026-09-04. Status: design accepted; provider selection,
target configuration, and the `API-01` activation window are pending.
...
[service-level objectives](service-level-objectives.md) measurable. It does not
activate that objective, select a vendor, or claim that the completed v2.3 soak
was a 30-day availability window.
...
| `execution_id` | Provider execution identifier used only for deduplication |
...
The next slice compares candidate managed synthetic-monitoring services using
the following gates:
...
1. **Contract**: accept this provider-independent design and synchronize the SLO
   register and roadmap.
2. **Selection**: compare providers against the checklist and record the choice
   and exit conditions.
3. **Target setup**: configure primary readiness and diagnostic liveness probes,
   external result retention, alert routing, and safe failure-path tests.
