...
accepted; shadow checks and the local normalized evidence pipeline are
implemented; bounded live export and independent archive/recovery rehearsals
passed. The scheduler environment, manual cycle, automatic archive, scheduled
segment recovery, and live `logs:read` validation are complete. The cadence
review now treats a mature missing result as a bad outcome rather than an
evidence-integrity failure. Alert routing is proved, and the fresh contiguous
24-hour shadow audit recorded for 2026-09-09 through 2026-09-10 passed. The
immutable activation tuple starts prospective `API-01` collection at
...
[provider decision](v2.4-synthetic-monitoring-provider-decision.md), and
Decision 23 defines the seven-day MVP objective window. The recorded activation
tuple moves only `API-01` into `Collecting`; it does not claim that the
completed v2.3 soak or current prospective window achieved the objective.

## Goals

- Produce one unambiguous result or explicit bad `missing` outcome for every
  UTC minute.
...
| Schedule | One expected UTC-minute slot; the 60-second provider check supplies the source result |
...
returned by the provider. A provider source timestamp is assigned to the UTC
minute that contains it. The provider need not execute at second zero, but the
same source result cannot satisfy more than one slot.
...
For a seven-day rolling window there are 10,080 expected minute slots:

```text
availability = good primary slots / 10,080
bad slots = 10,080 - good primary slots
draft target = availability >= 99.5% and bad slots <= 50
```
...
The seven-day period is an explicit first-MVP trade-off: it produces a
reviewable result sooner and a more responsive rolling signal, but it has more
week-to-week variance and is weaker evidence of long-term reliability than the
original 30-day proposal. The unchanged 45-day evidence-retention boundary
keeps several complete windows available for later review.
...
3. **Collecting**: a 24-hour shadow run has 1,440 mature evaluated slots,
   stable normalization, and meets the shadow-scale target with every missing
   result retained as bad. Record the activation timestamp, owner, primary
   location, monitor revision, evaluator revision, dashboard or report query,
   and alert route. The official denominator starts at that timestamp.
4. **Reviewable**: one uninterrupted prospective seven-day window has completed.
...
shadow rollout. Decision 22 confirms that the provider's frequency-based time
points can feed the generated UTC-slot denominator: source results stay in
their containing minute and absent mature results remain bad. Its built-in
retention does not by itself satisfy the 45-day raw-evidence gate. The rollout
has proved normalization, independent Backblaze B2 archival, and alert routing.
A fresh revised-policy shadow remains before `API-01` can advance. The complete
comparison, cost boundary, credential model, and exit conditions are in the
...
3. **Target setup (completed 2026-09-09)**: the primary and diagnostic shadow
   checks were configured on 2026-09-04. The normalized exporter, create-only
   local JSONL, deterministic evaluator, and contract fixtures are implemented.
   A bounded least-privilege live export has passed. A content-addressed B2
   upload,
   sequential duplicate suppression, SHA-256 verified download, and deterministic
   report comparison have also passed against a separate private archive.
   The scheduler environment, manual cycle, automatic archive, and independent
   scheduled-segment recovery are complete. A bounded Loki adapter and safe
   fixtures cover provider transport-failure detail, and live `logs:read`
   access has verified the expected labels and log format. An isolated
   failure/recovery exercise proved alert routing and its three-bad/two-good
   hysteresis without disrupting production. The sanitized effective
   configuration and exporter workflow are in
   `docs/v2.4-synthetic-monitoring-rollout.md` and
   `docs/v2.4-availability-evidence-exporter.md`.
4. **Shadow validation and activation (completed 2026-09-10)**: the fresh
   window recorded in `docs/v2.4-synthetic-monitoring-rollout.md` passed with
   1,440 mature evaluated good slots. The activation tuple starts a new
   prospective denominator without importing shadow samples.
5. **Evaluation and presentation**: the deterministic evaluator is implemented;
   only sanitized aggregates may enter the public record after the complete
   prospective window passes its maturity boundary.
6. **First review**: after a complete forward-looking seven-day window, publish the
...
| Slot population | A generated 24-hour fixture evaluates exactly 1,440 mature slots independent of exported row count; an absent source result is one bad `missing` slot |
| Budget boundary | 50 bad slots pass 99.5%; 51 bad slots fail it in a 10,080-slot window |
