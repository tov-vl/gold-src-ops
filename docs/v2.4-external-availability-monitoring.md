...
passed;
transport classification, alert routing, and the `API-01` activation window are
pending.
...
3. **Target setup (in progress)**: the primary and diagnostic shadow checks were
   ...
   External scheduling, provider transport-failure detail, and alert routing
   remain. The sanitized effective configuration and exporter workflow are in
