...
implemented; live export proof, independent retention, and the `API-01`
activation window are pending.
...
3. **Target setup (in progress)**: the primary and diagnostic shadow checks were
   ...
   deterministic evaluator, and contract fixtures are implemented. Live export
   proof, external result retention, provider transport-failure detail, and
   alert routing remain. The sanitized effective configuration and exporter
   workflow are in `docs/v2.4-synthetic-monitoring-rollout.md` and
