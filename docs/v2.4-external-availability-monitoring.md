...
24-hour shadow audit recorded for 2026-09-09 through 2026-09-10 is active. That
audit remains the gate before the prospective `API-01` activation window can
begin.
...
Decision 23 defines the seven-day MVP objective window. None of these decisions
activates the objective or claims that the completed v2.3 soak was a seven-day
availability window.
...
4. **Shadow validation (active 2026-09-09)**: collect the fresh window recorded
   in `docs/v2.4-synthetic-monitoring-rollout.md`, verify 1,440 mature evaluated
   slots and target attainment with missing retained as bad, then record the
   activation tuple without importing the shadow samples.
5. **Evaluation and presentation**: the deterministic evaluator is implemented;
   expose only sanitized aggregates to the later public dashboard after the
   measurement and retention gates pass.
