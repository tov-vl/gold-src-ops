...
- v2.15: begin by bounding the Reader Recent Activity investigation to an
  ...
  slices are integrated through pull requests #169 and #170. Signed candidate
  `v2.15.0-rc.1` passed immutable publication, an API/Web-only exact-digest
  rollout, four healthy samples over 182 seconds, and authenticated window and
  context navigation. Two reviewed deferred alert-outbox records remained
  unchanged with delivery disabled. Signed stable `v2.15.0` promotes the exact
  accepted API and Web digests without rebuilding;
