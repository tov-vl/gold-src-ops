...
internal sealed class OperatorRconCredentialConfirmationStore(TimeProvider timeProvider)
{
    ...
    public string? Issue(string subject, OperatorRconCredentialDraft draft)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(subject);
        ArgumentNullException.ThrowIfNull(draft);
        if (draft.ServerId == Guid.Empty ||
            draft.ExpectedServerRevision <= 0 ||
            draft.ExpectedCredentialRevision < 0 ||
            string.IsNullOrWhiteSpace(draft.SecretAlias))
        {
            throw new ArgumentException("Credential draft is invalid.", nameof(draft));
        }
        ...
        lock (sync)
        {
            ...
            confirmations.Add(token, new Confirmation(subject, draft, now + Lifetime));
            return token;
        }
    }
    ...
    public bool TryConsume(
        ...
        string subject,
        Guid serverId,
        out OperatorRconCredentialDraft draft)
    {
...
internal sealed record OperatorRconCredentialDraft(
    Guid ServerId,
