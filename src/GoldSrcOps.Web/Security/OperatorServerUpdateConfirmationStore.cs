...
internal sealed class OperatorServerUpdateConfirmationStore(TimeProvider timeProvider)
{
    ...
    public string? Issue(string subject, OperatorServerUpdateDraft draft)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(subject);
        ArgumentNullException.ThrowIfNull(draft);
        if (draft.ServerId == Guid.Empty)
        {
            throw new ArgumentException("Server ID must not be empty.", nameof(draft));
        }

        if (draft.ExpectedRevision <= 0)
        {
            throw new ArgumentException("Expected revision must be positive.", nameof(draft));
        }
    ...
    public bool TryConsume(
        ...
        string subject,
        Guid serverId,
        out OperatorServerUpdateDraft draft)
    {
...
internal sealed record OperatorServerUpdateDraft(
    Guid ServerId,
