...
internal sealed class GameEventRetentionOptions
{
    private const string SectionName = "GameEventRetention";

    public bool Enabled { get; init; } = true;

    public TimeSpan RetentionPeriod { get; init; } = TimeSpan.FromDays(45);

    public TimeSpan CleanupInterval { get; init; } = TimeSpan.FromMinutes(5);

    public int BatchSize { get; init; } = 1_000;

    public static GameEventRetentionOptions FromConfiguration(IConfiguration configuration)
    {
        var section = configuration.GetSection(SectionName);

        return new GameEventRetentionOptions
        {
            Enabled = ReadBool(section, "Enabled", defaultValue: true),
            RetentionPeriod = TimeSpan.FromDays(
                ReadInt(section, "RetentionDays", defaultValue: 45, minValue: 1, maxValue: 3_650)),
    ...
    private static InvalidOperationException InvalidValue(string key, string value, string expected) =>
        new($"Configuration value '{SectionName}:{key}' must be {expected}; received '{value}'.");
}
